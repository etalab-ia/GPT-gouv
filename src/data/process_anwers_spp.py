...
def process_answers(df: pd.Series) -> pd.Series:
    def filter_one_answer(l: List[str]) -> List[str]:
        return [
            text
            for text in l
            if "bonjour" not in text.lower()
            and "merci" not in text.lower()
            and "heureux" not in text.lower()
            and "le service public concerné" not in text.lower()
    ...
    def process_one_answer(row: str) -> str:
        ...
        return " ".join(filter_one_answer(answers))

    return df.apply(lambda row: process_one_answer(row))


path = (
    "../../data/raw/___liste_des_experiences_publiees_2023-05-09T14_25_40.084962Z.csv"
)

df = (
    pd.read_csv(path, sep=",")
    .drop_duplicates(subset=["ID"])
    .dropna(subset=["rep_administration"])
)
...
df_out.dropna(subset=["processed_answers"]).to_csv(
    "../../data/processed/processed_answers_spp.csv", sep="~", index=False
)
